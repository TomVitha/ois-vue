<script setup lang="ts">
  import { computed } from 'vue'

  const props = defineProps<{
    title: string
    date?: string
    body?: string
    isUnread?: boolean
    imgURL?: string
    category?: string
    
  }>()

  // Map category to icon SVG and color class
  const categoryIconsMap: Record<string, { code: string; color: string }> = {
    welcome: {
      code: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" class="icon icon-tabler icons-tabler-filled icon-tabler-confetti"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M3 5a1 1 0 0 1 1 -1a1 1 0 0 1 1.993 -.117l.007 .117a1 1 0 0 1 .117 1.993l-.117 .007a1 1 0 1 1 -2 0a1 1 0 0 1 -1 -1m7.53 -1.243a1 1 0 1 1 1.94 .486l-.5 2a1 1 0 1 1 -1.94 -.486zm6.47 1.243a1 1 0 0 1 1 -1a1 1 0 0 1 1.993 -.117l.007 .117a1 1 0 0 1 .117 1.993l-.117 .007a1 1 0 0 1 -2 0a1 1 0 0 1 -1 -1m-8.81 4.293l6.517 6.518a1 1 0 0 1 -.29 1.617l-9.573 4.387a2 2 0 0 1 -2.661 -2.652l4.39 -9.58a1 1 0 0 1 1.616 -.29m7.517 -1a1 1 0 0 1 0 1.414l-1 1a1 1 0 0 1 -1.414 -1.414l1 -1a1 1 0 0 1 1.414 0m4.05 3.237a1 1 0 0 1 .486 1.94l-2 .5a1 1 0 0 1 -.486 -1.94zm-2.756 7.47a1 1 0 0 1 1 -1a1 1 0 0 1 1.993 -.117l.007 .117a1 1 0 0 1 .117 1.993l-.117 .007a1 1 0 0 1 -2 0a1 1 0 0 1 -1 -1"></path></svg>`,
      color: 'green'
    },
    promo: {
      code: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-bulb"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M3 12h1m8 -9v1m8 8h1m-15.4 -6.4l.7 .7m12.1 -.7l-.7 .7"></path><path d="M9 16a5 5 0 1 1 6 0a3.5 3.5 0 0 0 -1 3a2 2 0 0 1 -4 0a3.5 3.5 0 0 0 -1 -3"></path><path d="M9.7 17l4.6 0"></path></svg>`,
      color: 'yellow'
    },
    important: {
      code: `<svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-alert-triangle"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 9v4" /><path d="M10.363 3.591l-8.106 13.534a1.914 1.914 0 0 0 1.636 2.871h16.214a1.914 1.914 0 0 0 1.636 -2.87l-8.106 -13.536a1.914 1.914 0 0 0 -3.274 0z" /><path d="M12 16h.01" /></svg>`,
      color: 'red'
    },
    default: {
      code: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-info-circle"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M3 12a9 9 0 1 0 18 0a9 9 0 0 0 -18 0"></path><path d="M12 9h.01"></path><path d="M11 12h1v4h1"></path></svg>`,
      color: 'blue'
    }
  }

  const icon = computed(() => {
    return categoryIconsMap[props.category ?? '']?.code || categoryIconsMap.default.code
  })
  const iconColor = computed(() => {
    return categoryIconsMap[props.category ?? '']?.color || categoryIconsMap.default.color
  })
</script>

<template>
  <a href="https://google.com" class="card">
    <!-- Image -->
    <div v-if="imgURL" class="img-responsive img-responsive-16x9 card-img-top d-lg-none" :style="{ backgroundImage: `url('${imgURL}')` }"></div>
    <div class="card-body">
      <div class="row align-items-center">
        <div class="col">
          <small class="text-secondary">
            <span v-if="isUnread" class="badge badge-dot bg-primary d-inline-block me-1"></span>
            {{ date }}
          </small>
          <h3 class="card-title mb-0">{{ title }}</h3>
        </div>
        <div class="col-auto">
          <div class="avatar" :class="`bg-${iconColor}-lt text-${iconColor}-lt-fg`">
            <span v-html="icon"></span>
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col text-secondary mt-0">{{ body }}</div>
      </div>
    </div>
  </a>
</template>